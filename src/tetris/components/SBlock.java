...
public class SBlock extends Block {
	private final int color = Colors.PINK;
	 ...
	 */
	public SBlock(float x, float y, float speed) {
		super();
		this.speed = speed;
	...
	public ArrayList<Square> generateSquares(float x, float y) {
		...
		mainSquare = new Square(color, x + BlockInfo.SIZE, y, speed);
	...
	public void editPosition(int newPosition) {
		...
		if (newPosition == 0) {
			...
			squares.get(3).setX(x).setY(y + BlockInfo.SIZE);
		} else if (newPosition == 1) {
			squares.get(1).setX(x).setY(y + BlockInfo.SIZE);
			squares.get(2).setX(x - BlockInfo.SIZE)
					.setY(y - BlockInfo.SIZE);
			squares.get(3).setX(x - BlockInfo.SIZE).setY(y);
	...
	public boolean canRotate(int dir) {
		SBlock block = new SBlock(getMainSquareXPos(), getMainSquareYPos(), 0);
